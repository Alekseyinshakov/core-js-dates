...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const firstStamp = new Date(dateStart).getTime();
  const secondStamp = new Date(dateEnd).getTime();
  const differentStamp = secondStamp - firstStamp;

  return differentStamp / (60 * 60 * 1000 * 24) + 1;
}
